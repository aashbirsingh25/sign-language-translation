...
def create_model(model, model_weights_path=None, top_model=True, color_mode="rgb", input_shape=None):
    ...
    if top_model:
        ...
        # Print expected top-model input (flatten) size to help debugging
        try:
            flatten_shape = model.output_shape[1:]
            from functools import reduce
            import operator
            expected_flatten = reduce(operator.mul, flatten_shape, 1)
            print(f"[DEBUG] base model output shape (per-sample): {model.output_shape}. expected flatten size: {expected_flatten}")
        except Exception:
            expected_flatten = None
        ...
        if model_weights_path is not None:
            ...
            try:
                ...
                # Provide a more actionable error message when shapes mismatch
                err_msg = str(e)
                hint = "The provided weights file is incompatible with the chosen base model or input shape."
                hint += "\nMake sure you're using weights that were created with the same base network (e.g. 'vgg16' weights with -m vgg16) and the same input image size."
                if expected_flatten is not None:
                    hint += f"\nExpected top dense kernel shape first-dim (flattened features) ~= {expected_flatten}."
                hint += "\nIf you don't have matching weights, either supply matching weights or omit -w to use the repo defaults (if available), or re-train the top classifier."
